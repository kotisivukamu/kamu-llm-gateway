...
--
-- Run as a superuser/admin (creates roles by name only, no password;
-- SECURITY DEFINER functions; grants). The app connects at runtime as
-- `app_user` (the RLS-scoped request pool, no bypass) for tenant-scoped
-- work, and as `admin_role` (LOGIN BYPASSRLS, NOT superuser, owns nothing)
-- for the trusted cross-tenant path (login reconcile, key hashing lookups,
-- provisioning, the proxy's cross-tenant key-hash lookup). Both roles are
-- created here WITHOUT a password: a LOGIN role with no password can't
-- authenticate, so it's inert until a password is set out-of-band by a
-- human (never committed, never generated by an agent — see
-- database/init-roles.example.sql), same convention as kamusites'
-- 20260610130000_admin_role.up.sql. No migration ever sets or alters a
-- password.
--
...
--   app_user      — LOGIN, NO BYPASSRLS. The RLS-scoped request pool: every
--                    tenant-facing query runs through withUserContext
--                    (SET LOCAL ROLE authenticated), so app_user itself must
--                    stay genuinely subject to RLS, not bypass it.
--   admin_role    — LOGIN, BYPASSRLS. The privileged runtime pool: login
--                    reconcile, key hashing lookups, provisioning, the
--                    proxy's cross-tenant key-hash lookup. NOT superuser,
--                    owns nothing.
...
--
-- app_user and admin_role are created here by name + capability flags only
-- (no PASSWORD clause) — see the header comment above and
-- database/init-roles.example.sql for the manual, out-of-band step that
-- sets real passwords.
-- ---------------------------------------------------------------------------
DO $$ BEGIN
...
-- admin_role: BYPASSRLS has to be set at CREATE ROLE time on PlanetScale
-- (ALTER ROLE ... BYPASSRLS is restricted there), which is why it's
-- specified inline here rather than via a later ALTER ROLE (mirrors
-- kamusites' 20260610130000_admin_role.up.sql).
DO $$ BEGIN
...
-- admin_role: same schema, BYPASSRLS makes it unnecessary to route through
-- authenticated/app_user grants, but it still needs ordinary object grants
-- (BYPASSRLS skips RLS checks, not GRANT checks) plus the sequence and
-- default-privileges wiring so future tables/sequences created by postgres
-- via later migrations auto-grant to admin_role too.
GRANT USAGE ON SCHEMA llm TO admin_role;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA llm TO admin_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA llm TO admin_role;
GRANT USAGE, SELECT ON SEQUENCE llm.usage_log_id_seq TO admin_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA llm
...
-- ---------------------------------------------------------------------------
-- RLS. teams and team_members are reconcile-owned via admin_role (BYPASSRLS
-- — app_user itself is NOT bypass and has no write path of its own here),
-- so authenticated gets read-only visibility. keys are editable by team members;
